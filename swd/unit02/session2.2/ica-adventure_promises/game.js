...
game.goToLocation = (locationName) => {
    if(map[player.location].exits.includes(locationName) && map[locationName] == undefined) {
        return new Promise((resolve, reject) => {
            var url = 'http://localhost:3000/';
            fetch(url.concat(locationName)).then((response) => {
                return response.text();
            }).then((emptyObj) => {
                let object = emptyObj;
                objectjs = JSON.parse(object);
                map[locationName] = objectjs;
                let locationDescription = objectjs.description;
                player.location = locationName;
                resolve(locationDescription);
            }).catch((error) => {
                reject(error);
            });
        });
    } else if (map[player.location].exits.includes(locationName)) {
        player.location = locationName;
